...
async def bot_send(bot: Bot, chat_id: int, caption: str, kb, fid=None, use_banner=False):
    """إرسال رسالة بنص فقط افتراضياً، أو مع بانر ثابت عند الطلب."""
    if use_banner and fid:
        try:
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await bot_send(message.bot, uid, cap, kb, fid, use_banner=True)
...
async def cmd_admin(message: Message, state: FSMContext):
    ...
    await bot_send(message.bot, uid,
                   "🔐 *لوحة الأدمن*\n\nأدخل كلمة المرور:",
                       kb_back(uid), fid, use_banner=True)
# ══════════════════════════════════════════════════════
#  /addgateway
# ══════════════════════════════════════════════════════
...
async def _handle_gw_wizard(message: Message, state: FSMContext, text: str, step: str):
    ...
    await bot_send(message.bot, uid, _GW_PROMPTS[step], cancel)
...
async def on_callback(query: CallbackQuery, state: FSMContext):
    ...
    if data.startswith("gw|"):
        ...
        if not card:
            ...
            return
        await _run_single(query, state, int(data.split("|")[1]), card)
        return
...
async def _run_single(query: CallbackQuery, state: FSMContext, gw_id: int, card: dict):
    uid = query.from_user.id
    bot = query.bot
    ...
    async with active_checks_lock:
        ...
        if current >= MAX_CONCURRENT_CHECKS:
            await bot_edit(query, "❌ لديك فحصان جاريان. انتظر الانتهاء أو الغِ أحدهما.", kb_back(uid))
            return
    ...
    try:
        ...
        await bot_edit(query, s(uid, "checking"), temp_kb)
...
async def _run_bulk(query: CallbackQuery, state: FSMContext):
    uid = query.from_user.id
    bot = query.bot
    data = await state.get_data()
    ...
    if not cards or not gw_id:
        await bot_edit(query, s(uid, "session_expired"), kb_back(uid))
        return
    ...
    try:
        for i, card in enumerate(cards, 1):
            ...
            if not is_allowed(uid):
                await state.update_data(bulk_cancel=True)
                try:
                    await bot_send(bot, uid, "⛔ انتهى اشتراكك. تم إيقاف الفحص فوراً.", None)
                except Exception:
                    pass
                break
            ...
            if show_card:
                ...
                try:
                    await bot_send(bot, uid, msg, InlineKeyboardMarkup(inline_keyboard=[]))
                except Exception as e:
